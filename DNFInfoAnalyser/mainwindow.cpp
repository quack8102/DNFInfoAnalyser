...
MainWindow::MainWindow(QWidget *parent)
    ...
    windowMenu->setTitle(QString::fromLocal8Bit("窗口"));
    grabAction->setText(QString::fromLocal8Bit("从游戏画面分析"));
    connect(grabAction, &QAction::triggered, this, &MainWindow::actionActivated);
    openAction->setText(QString::fromLocal8Bit("从截图文件分析..."));
    connect(openAction, &QAction::triggered, this, &MainWindow::on_openBtn_clicked);
    setAction->setText(QString::fromLocal8Bit("设置..."));
    ...
    updateAction->setText(QString::fromLocal8Bit("下载最新版..."));
    ...
    if (SettingData::setflag("use")) {
        SettingData::sendMessage(QString::fromLocal8Bit("<p>欢迎您使用DNF面板计算器，点击<a href=\"https://quack8102.gitee.io/#/quickstart\">这里</a>查看快速使用教程。</p>"));
    }
...
void MainWindow::on_resultupdate(QNetworkReply *reply) {
    if (m_currentReply->error() != QNetworkReply::NoError) {
        ...
        mainTray->showMessage(QString::fromLocal8Bit("DNF面板计算器"), //消息窗口标题
                              QString::fromLocal8Bit("程序已启动\n检查更新失败\n点击以打开疑难解答"), //消息内容
    ...
    //qDebug() << value;
    mainTray->showMessage(QString::fromLocal8Bit("DNF面板计算器"), //消息窗口标题
                          QString::fromLocal8Bit("程序已启动\n当前版本：") + version + QString::fromLocal8Bit("，最新版本：") + value.toString(), //消息内容
                          QSystemTrayIcon::MessageIcon::Information, //消息窗口图标
                          1000); //消息窗口显示时长
}
...
void MainWindow::refresh() {
    ...
    if (vec.empty()) {
        currentHWND = NULL;
        QAction *tmpAction = new QAction(QString::fromLocal8Bit("无"));
        ...
        for (HWND v : vec) {
            ...
            QAction *tmpAction;
            if (v == currentHWND) {
                tmpAction = new QAction(QString::asprintf("Window %d", tot) + QString::fromLocal8Bit(" √"));
            } else {
...
void MainWindow::readImg(Mat &inMat, const QString &path) {
    ...
    if (ret) {
        try {
            ...
            QMessageBox::critical(NULL, QString::fromLocal8Bit("错误"), QString::fromLocal8Bit("图片转换失败"));
        }
    } else {
        qCritical() << "Failed to read the image at " + path + ".";
        QMessageBox::critical(NULL, QString::fromLocal8Bit("错误"), QString::fromLocal8Bit("图片读取失败"));
    }
}
...
void MainWindow::grabDNFWindow(const HWND &hwnd) {
    if (hwnd == NULL) {
        ...
        mainTray->showMessage(QString::fromLocal8Bit("错误"), //消息窗口标题
                              QString::fromLocal8Bit("当前未开启DNF窗口\n点击以打开疑难解答"), //消息内容
...
void MainWindow::on_openBtn_clicked() {
    QString fileName = QFileDialog::getOpenFileName(
                           ...
                           "./",
                           QString::fromLocal8Bit("图片文件(*.png *.jpeg *.jpg *.bmp)"));
    if (!fileName.isEmpty()) {
        Mat inMat;
